...
public class GameManager : Singleton<GameManager>
{
    ...
    void UpdateWaitProcess()
    {
        ...
        if ((m_dicPlayerController.Count >= 2 && m_bPropertyIsAutoRole) ||
            (!m_bPropertyIsAutoRole && (m_dicPlayerController.Count >= (m_iPropertyNumberOfMafia + Mathf.Max(1, m_iPropertyNumberOfDetective)))))
        {
            if (PhotonNetwork.IsMasterClient)
            {
